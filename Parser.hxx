...
constexpr auto strToInt( InputIt first, InputIt last, Int& res, int base )
	-> std::enable_if_t<std::is_integral<Int>{}, ParserState<InputIt>>
{
...
constexpr auto strToFloat( InputIt first, InputIt last, Float& res )
	-> std::enable_if_t<std::is_floating_point<Float>{}, ParserState<InputIt>>
{
	String digits = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
constexpr auto strToFloat( char const* str, std::size_t len, std::size_t* pos = 0 )
	-> std::enable_if_t<std::is_floating_point<Float>{}, Float> {
	Float ret = 0;
